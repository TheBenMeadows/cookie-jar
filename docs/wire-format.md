...
The jar page is `#/jar/<recipient>`. Narrowed to one reference it is the receipt page: `#/jar/<recipient>?ref=<reference>`, with the reference URL-encoded. A reference is settled when the payments tagged with it, in the requested token, reach the requested amount. Below that amount it is part-paid.

With none arrived there are two answers, and an implementation of this format must tell them apart. The reference is **unpaid** only when the read covered the jar: it consumed every signature the node returned, it did not stop at its own scan cap or payment limit, and its oldest signature sits clear of the node's earliest retained block. Otherwise the reference is **unknown**, rendered as no record. The third condition is the one that is easy to miss: a node keeps a rolling window, so a jar older than that window ends its history at the node's earliest block rather than at its own first payment, and a read that consumed everything on offer has still seen only the last ten days. Cookie Tab compares the oldest signature's slot against `getFirstAvailableBlock` and treats anything within 216,000 slots, about a day, as the floor. A match is evidence wherever it is found, so a truncated read can still report **paid**; only the absence is conditional.
